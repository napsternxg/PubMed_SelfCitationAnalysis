...
#MODEL_SUFFIX = "first_author"
MODEL_SUFFIX = "last_author"
...
store_path, X_df_path, y_df_path = "out/Model.v3.%s.h5" % MODEL_SUFFIX, "X", "y"
...
if include_prc:
  ...
  fig.tight_layout()
  plt.savefig("PRC_model.v3.%s.pdf" % MODEL_SUFFIX, bbox_inches="tight", bbox_extra_artists=[lgd])
...
lgd = fig.legend(*ax[0].get_legend_handles_labels(), loc = 'upper center', bbox_to_anchor=(0.5,1.1), ncol=5, frameon=True, fancybox=True, prop={"size": 9})
fig.tight_layout()
plt.savefig("Measures_model.v3.%s.pdf" % MODEL_SUFFIX, bbox_inches="tight", bbox_extra_artists=[lgd])


## Generate intercept model
dump(results_full, "results_all_model/v3/results_all_model.v3.%s.pkl" % MODEL_SUFFIX)
...
dump([[results_intercept]] + results_full, "results_all_model/v3/results_all_model_intercept.v3.%s.pkl" % MODEL_SUFFIX)
# DONE
...
with open("Full_measures_coeffs_model.v3.%s.txt" % MODEL_SUFFIX, "wb+") as fp:
  for j,(r,tf) in enumerate(zip(results_full[1:], TOP_FEATURES[:-1])):
...
gender_ci_975 = []
index_names = []
for i, (r,tf) in enumerate(zip(results_full[1:], TOP_FEATURES)):
...
plt.savefig("Gender_coeffs_model.v3.%s.pdf" % MODEL_SUFFIX, bbox_inches="tight")
